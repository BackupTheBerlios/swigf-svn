...
public class HelloJava extends UIApplication {
	...
	@Message
	UITableCell table$cellForRow$column$reusing$(UITable table, int row,
			UITableColumn col, UITableCell reusing) {
		...
		if (reusing != null)
			...
			cell = (UIImageAndTextTableCell) new UIImageAndTextTableCell()
					.init();
	...
	public void applicationDidFinishLaunching$(Object unused) throws Exception {
		...
		ab.open(userHomeDirectory().toString()
				+ "/Library/AddressBook/AddressBook.sqlitedb", 0666);
		try {
			SQLite.Stmt st = ab
					.prepare("select first, last from ABPerson where first is not null order by first");
			try {
				while (st.step())
					contacts_.add(new Contact(st.column_string(0), st
							.column_string(1)));
			} finally {
				st.close();
			}
		...
		CGRect lower = new CGRect(0, navsize.height, inner.size.width,
				inner.size.height - navsize.height);
		UISectionList list = new UISectionList().initWithFrame$(lower);
		view.addSubview$(list);

		UITableColumn col = new UITableColumn()
				.initWithTitle$identifier$width$("Name", "name", 320);
		...
		/*
		 * // XXX: this doesn't work the same on 2.0 and I don't want to think
		 * about porting it right now AVController controller = new
		 * AVController().init(); CharSequence wavfile = (CharSequence)
		 * ((NSBundle) NSBundle.mainBundle()).pathForResource$ofType$("start",
		 * "wav"); AVItem wavitem = new AVItem().initWithPath$error$(wavfile,
		 * null); wavitem.setVolume$(100); controller.setCurrentItem$(wavitem);
		 * controller.setCurrentTime$(0); controller.play$(null);
		 */
	}
